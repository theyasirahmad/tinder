...
class Dashboard extends Component {
    ...
    componentDidMount() {
        ...
        db.collection('meetings')
            .where('reciever', "==", `${userinfo.uid}`).where("status", "==", "pending")
            .get()
            .then((snapshot) => {
                snapshot.forEach((doc) => {
    ...
    meetingsStautsRender() {
        ...
        const { meetings, requestedMeetings } = this.state;
        var allmeetings;
        if (meetings) {
            allmeetings = meetings
        }
        if (requestedMeetings) {
            allmeetings = requestedMeetings
        }
        if (meetings && requestedMeetings) {
            allmeetings = meetings;
            allmeetings.push(...requestedMeetings)
        }
                            ...
                            return (
                                <div key={index} className={"col-xs-12 col-sm-6 col-md-4 col-lg-3"}>
                                    <div className={"meetingstatusdiv "} key={index}>
                                        ...
                                        <button className={"btn btn-default"} onClick={() => { this.addtocalanderFunc(event) }}>
    ...
    render() {
        const { meetings, requestedMeetings, selectedPlace } = this.state;
